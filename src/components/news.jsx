...
export default function ImageWithTextSection() {
  ...
  const news = [
    ...
    {
      id: 2,
      title: "Innovative Solutions by Company X Highlighted in Igihe",
  ...
  return (
    <section className="py-20 bg-[#f4f4f4]">
      <div className="container mx-auto text-center px-4 sm:px-8"> {/* Added padding for mobile */}
        <h2 className="text-3xl font-bold text-[#213348] mb-8 font-[Monospace]">Latest News</h2>
          ...
          {news.map(item => (
            ...
            >
              ...
              <Link href={item.link} passHref className="relative block"> {/* Wrap the whole item in a clickable link */}
                ...
                <div className="absolute bottom-4 left-4 text-white font-[Monospace] group-hover:opacity-0 transition-opacity duration-300 z-20">
                  <h3 className="text-sm font-semibold">{item.title}</h3>
                </div>

                {/* Hover effect for description: show description when hovered */}
                <div className="absolute bottom-4 left-4 text-white font-[Monospace] opacity-0 group-hover:opacity-100 transform group-hover:translate-y-0 transition-all duration-300 z-20">
